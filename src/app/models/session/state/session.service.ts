...
export class SessionService {
  ...
  login(creds) {
    ...
    this.http.post(`${environment.api_url}/users/sign-in`, creds).subscribe(
      (res: any) => {
  ...
  refreshAccessToken() {
    return new Observable((observer) => {
      return this.http
        .post(this.AUTH_API_URL + "/users/refresh-auth", {
          token: window.localStorage.getItem("sb_refreshtoken"),
        })
        .subscribe(
          (response: any) => {
  ...
  recoverSession(token) {
    this.http
      .post(this.AUTH_API_URL + "/users/refresh-auth", { token: token })
      .subscribe(
        (res: any) => {
          this.sessionStore.update((state) => {
  ...
  getUserDetails() {
    ...
    this.http
      .get(this.AUTH_API_URL + "/users/me", { headers: headers })
      .subscribe(
        (res: any) => {
          this.sessionStore.update((state) => {
