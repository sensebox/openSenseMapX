...
import {
  processBoxData,
  toGeoJson,
} from "src/app/modules/explore/box/osem-line-chart/helper/helpers";
...
export class BoxService {
  ...
  get() {
    ...
    return this.http
      .get<Box[]>(
        `${environment.api_url}/boxes?classify=true&bbox=13.0882097323,52.3418234221,13.7606105539,52.6697240587&full=true`
      )
      .pipe(
        tap((entities) => {
  ...
  getValues(pheno, dateRange, bbox) {
    ...
    return this.http
      .get<any[]>(
        `${
          environment.api_url
        }/statistics/descriptive?&phenomenon=${pheno}&bbox=${bboxString}&format=json&columns=boxId,lat,lon,boxName,exposure&from-date=${dateRange[0].toISOString()}&to-date=${dateRange[1].toISOString()}&window=3600000&operation=arithmeticMean`
      )
      .pipe(
        tap((entities) => {
          entities = entities.map((ent) => {
            ...
            return {
              ...
              lat: ent.lat,
              lon: ent.lon,
              sensors: [{ _id: sensorId, title: pheno }],
              values: {
                [pheno]: noEnt,
              },
            };
  ...
  getSingleBox(id) {
    return this.http.get<Box>(`${environment.api_url}/boxes/${id}`).pipe(
      tap((entity) => {
  ...
  getMyBoxes() {
    ...
    this.http
      .get(this.AUTH_API_URL + "/users/me/boxes", { headers: headers })
      .subscribe((res: any) => {
  ...
  setActive(id) {
    if (id) this.getSingleBox(id).subscribe();
