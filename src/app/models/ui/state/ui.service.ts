...
export class UiService {
  ...
  fetchStats() {
    return this.http.get(`${environment.api_url}/stats`).pipe(
      tap((stats) => {
        this.uiStore.update((state) => ({
          ...state,
          stats: { totalBoxes: stats[0], totalMeasurements: stats[1] },
        }));
      })
    );
  }
  ...
  fetchGeocodeResults(searchstring) {
    ...
    return this.http
      .get("//locationiq.org/v1/search.php", { params: params })
      .pipe(first())
      .pipe(
        tap((res: any) => {
          this.uiStore.update((state) => ({
            ...state,
            locationAutocompleteResults: res,
          }));
        })
      );
  }
  ...
  toggleFilterVisible() {
    this.uiStore.update((state) => ({
      ...state,
      fitlerVisible: !state.fitlerVisible,
    }));
  }
      ...
      clusterLayers: [
        {
          ...
          layout: {
            ...state.baseLayer.layout,
            visibility:
              !state.clustering && !state.selectedDate ? "visible" : "none",
          },
        },
        {
          ...state.clusterLayers[1],
